...
export function ChatForm({ className, ...props }: React.ComponentProps<"form">) {
  ...
  const handleSubmit = async(e: React.FormEvent<HTMLFormElement>) => {
    ...
    setInput("")
    const res = await axios.post("http://127.0.0.1:8000/health", {
      query: input,
    })
    console.log(res)
    const {response} = await res.data
    append({ content: response?.raw, role: "assistant" })
  }
      ...
      >
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <Button variant="ghost" size="sm" className="absolute  right-1 size-6 rounded-full">
              <ArrowUpIcon size={16} />
            </Button>
          </TooltipTrigger>
